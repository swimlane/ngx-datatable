...
export class RowGroupingComponent {
  ...
  checkGroup(event, row, rowIndex, group) {
    ...
    if (group.filter(rowFilter => rowFilter.exppaypending === 1).length === 0 
      ...
      const numberOfSourceFunder = group.filter(rowFilter => rowFilter.exppayyes === 1 
                                                && rowFilter.source === 'Funder').length;
      const numberOfSourceCalculated = group.filter(rowFilter => rowFilter.exppayyes === 1 
                                                    && rowFilter.source === 'Calculated').length;
      const numberOfSourceManual = group.filter(rowFilter => rowFilter.exppayyes === 1 
                                                && rowFilter.source === 'Manual').length;
